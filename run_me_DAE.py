...
repeated_model_num = 5
train_sdae_eps = 3000 #EPOCHS FOR REPRESENTATION LEARNING
train_sm_eps = 5000 #EPOCHS FOR TRAINING CLASSIFIER
train_ft_eps = 8000 #EPOCHS FOR FINE-TUNING
...
n_code_h = 250 #NUMBER OF HIDDEN NODES FOR LATENT REPRESENTATIONS
...
for model_num in xrange(repeated_model_num):
	...
	np.savetxt("prediction_of_me_DAE_m" + str(model_num) + "_" + str(max_accr) + "_" + str(n_features) + "_features", max_pred, fmt="%.0f", delimiter=",")
	np.savetxt("label_m" + str(model_num) + "_" + str(max_accr) + "_" + str(n_features) + "_features", max_label, fmt="%.0f", delimiter=",")
	max_accr= 0.0
